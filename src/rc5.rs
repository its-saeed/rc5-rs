...
pub fn encode<W: Word>(key: &[u8], plaintext: &[u8], rounds: usize) -> Result<Vec<u8>, Error> {
...
pub fn decode<W: Word>(key: &[u8], ciphertext: &[u8], rounds: usize) -> Result<Vec<u8>, Error> {
    let word_bytes = size_of::<W>();
    let block_size = 2 * word_bytes;
...
fn decode_block<W: Word>(expanded_key: &[W], mut block: [W; 2]) -> Result<[W; 2], Error> {
    ...
    for i in 1..=num_rounds {
        let rotation =
            block[1].to_u128().ok_or(Error::InvalidWordSize)? % W::NUMBER_OF_BITS as u128;
        block[0] = (block[0].bitxor(block[1]))
            .rotate_left(rotation as u32)
            .wrapping_add(&expanded_key[2 * i]);

        let rotation =
            block[0].to_u128().ok_or(Error::InvalidWordSize)? % W::NUMBER_OF_BITS as u128;
...
fn encode_block<W: Word>(expanded_key: &[W], mut block: [W; 2]) -> Result<[W; 2], Error> {
    let num_rounds = (expanded_key.len() / 2) - 1;
...
pub fn expand_key<W: Word>(key: &[u8], rounds: usize) -> Result<Vec<W>, Error> {
    ...
    for _ in 0..iters {
        ...
        let rotation =
            a.wrapping_add(&b).to_u128().ok_or(Error::InvalidWordSize)? % W::NUMBER_OF_BITS as u128;
